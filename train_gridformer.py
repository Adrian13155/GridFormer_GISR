...
from basicsr.archs.GridFormer_arch import GridFormer
    ...
    try:
        ...
        for train_data_lists in pbar:
            for task_id, data_list in enumerate(train_data_lists):
                ...
                pred_image = model(inp_lr, inp_guide)[-1]
                
                pix_loss = cri_pix(pred_image, inp_gt)
                if inp_gt.shape[1] == 4:
                    l_percep_rgb, _ = cri_perceptual(pred_image[:,0:3,:,:], inp_gt[:,0:3,:,:])
                    l_percep_ir, _ = cri_perceptual(pred_image[:,3:4,:,:].repeat(1,3,1,1), inp_gt[:,3:4,:,:].repeat(1,3,1,1))
                    perceptual_loss =  l_percep_rgb + l_percep_ir
                else:
                    perceptual_loss, _ = cri_perceptual(pred_image.repeat(1,3,1,1), inp_gt.repeat(1,3,1,1))
        ...
        for dataset_id, dataset in enumerate(datasets):
            ...
            if dataset_id <= 2:  # depth estimation tasks
                for index, (inp_lr, inp_gt, inp_guide) in enumerate(tqdm(val_dataloader, desc=f'Validating Depth_{4*(2**dataset_id)}')):
                    output = model(inp_lr.cuda(), inp_guide.cuda())[-1][0]
                ...
                for inp_lr, inp_gt, inp_guide in tqdm(val_dataloader, desc=f'Validating Task_{dataset_id}'):
                    output = model(inp_lr.cuda(), inp_guide.cuda())[-1]
                    metric_values.append(
...
def main(opt):
    ...
    # --- Define the network --- #
    Generator = GridFormer().cuda()
    optimizer_G = torch.optim.AdamW(Generator.parameters(), lr=opt.learning_rate,betas=[0.9,0.999], weight_decay=1e-4)
    lr_scheduler_G = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_G, T_max=1200000, eta_min=1e-6)
